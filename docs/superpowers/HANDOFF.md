...
| **The About section renders the hero's portrait a second time** | [`src/sections/About.tsx`](../../src/sections/About.tsx) | The owner has called it redundant and it is: the same `/profile/avatar.webp`, twice on one page, measured. What replaces it is an open question — see [the About column](#the-about-column-is-an-open-question). |
...
### The About column is an open question

**The About section renders `/profile/avatar.webp` — the exact file the hero's `ProfileCard` renders.** Measured on the built page: two `<img>` elements, one `src`. The owner spotted it and asked what could go in that column instead; nothing has been decided, so the photo is still there.
