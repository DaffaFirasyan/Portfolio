...
- **Three checks are still open, all needing a browser the owner drives.** The keyboard pass end to end including the two dialog behaviours above, the reduced-motion pass with Windows animation effects off, and that the `Galaxy` backdrop resizes with the window — unprovable in the pane, see the environment note above. Nothing here is known to be broken; it is unmeasured, which is not the same thing.
