...
- **Four more checks are open, all needing a browser the owner drives.** The contact-and-launch plan's Task 9 covers them: the keyboard pass end to end, the reduced-motion pass with Windows animation effects off, that the `Galaxy` backdrop resizes with the window (unprovable in the pane, see the environment note above), and Lighthouse against spec §12.3. Nothing here is known to be broken — it is unmeasured, which is not the same thing.
