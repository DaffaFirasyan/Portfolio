...
| ~~**AssetMind's thumbnail**~~ | — | **Supplied by the owner 2026-08-22.** A real CMMS login capture. It arrived as a 2,556 KB PNG named `.webp` at 2880×1800 and was re-encoded to 800×500 WebP, 29 KB — a 98.9% saving on a featured row. |
| ~~**AssetMind's stack**~~ | — | **Supplied 2026-08-22**: six of the eighteen the owner listed, one per layer. |
| ~~**`village-portal.webp` is the wrong screenshot**~~ | — | **Fixed 2026-08-22** from `Konten_Asli/gambar/projects/Profil Desa.png`. It was byte-identical to `simpel-ibs.webp` and showed the SimpelBS homepage on the 01 featured row. |
| ~~**`Neo4j` and `FastAPI` missing from Skills**~~ | — | **Added 2026-08-22**, both `intermediate` and both linked to `assetmind`. The `level` on each is a guess held deliberately low — one word to change. |
| ~~**About renders the hero's portrait a second time**~~ | — | **Resolved 2026-08-22.** The IEEE paper took the column; the page now carries exactly one avatar `<img>`, measured. See [the About column](#the-about-column). |
...
| ~~**The `web-developer` certificate title**~~ | — | **Confirmed 2026-08-22**: "Web Developer — Programming and Software Development". Nothing in `src/data/` is unverified now. |
...
**Projects is one bento grid as of 2026-08-22, and `FeaturedProject`, `ProjectFlow` and `FlowingMenu` are all deleted.** Importance used to be expressed by *using a different component per tier* — 323px `FeaturedProject` rows for the featured three, 76px `FlowingMenu` rows for the rest — which is why the section read as two unrelated things stacked on each other. It is expressed by **area** now: `featured` becomes a wider cell in the same grid, alternating so each row sums to three columns.

The interaction is Aceternity's **Focus Cards**, vendored as a *mechanic* rather than a card. Upstream owns everything — a fixed `h-60 md:h-96`, a hardcoded `grid-cols-3 max-w-5xl`, `bg-gray-100 dark:bg-neutral-900`, an untyped `card: any`, a bare `<img>`, and a title in a `<div>`. Taken whole it would have imposed a height on the one section this project keeps cutting down, broken the heading outline `projects-filter.test.tsx` asserts, and reintroduced exactly the hardcoded palette `SpotlightCard` already had edited out of it. What was kept is the twelve lines that matter: one index is hovered, everything else softens. **It rhymes with the skill-to-project cross-highlight deliberately**, so the two read as one language; the softening here is gentler because one is an answer to a question the reader asked and the other is just the cursor moving.

**Lightswind was evaluated for the same job and lost on availability.** Its `focus-cards`, `cool-bento-effect`, `bento-grid` and `split-image-grid` are **Pro** — the registry returns `403` with a subscription notice. Of what is free, `glowing-cards` and `magic-card` both duplicate `Surface`'s cursor spotlight, which is the exact objection that rejected `MagicBento`, and `interactive-card` wants `framer-motion` — the old name for `motion`, already eager in this bundle, so it would have shipped the same library twice.
...
**The `AI & Retrieval` skills now point at something.** That whole category had no `relatedProjectIds` at all, because the only AI project on the page was the unwritten Dukunify slot — so hovering RAG, KG-RAG, Knowledge Graphs or LLM Applications dimmed the entire Projects section and highlighted nothing. Six of the seven link to `assetmind` now, plus `Python`. Only `NLP` stays unlinked, because it is the one skill in that category AssetMind's own account never names.

Embeddings and Sentence Transformers were left out on the first pass, on the reasoning that they were evidenced for the Pertamina work rather than this one. The owner's stack table then showed `sentence-transformers` and PyTorch in AssetMind too, so that reasoning was superseded rather than wrong — which is the normal way a link like this should change: on evidence, not on a hunch.

**It took Animart's featured slot on the same day**, at the owner's decision after it was put to him. `featuredMax` is 3 and all three held Laravel builds; of those three, Animart's moving-average forecast was the weakest technically, so it is the one that moved to the quiet tier. The featured order is now 01 village portal, 02 Simpel IBS, 03 AssetMind, which is array order — **leading with AssetMind is a one-line move** and has not been made.

**The stack is six of the eighteen the owner listed**, chosen so each names a different layer rather than a different package: `Python`, `FastAPI`, `LangChain`, `Llama 3.3 70B`, `Neo4j`, `Sentence Transformers`. Uvicorn, Pydantic, pytest, httpx and the `langchain-*` adapters are real and implied by those six — a chip row listing a test runner beside a 70B model tells a reader less, not more. Neo4j earns its slot over anything competing for it because the knowledge graph is the whole claim of the project, and it is the only chip that says the graph is real rather than a diagram.

**The thumbnail needed re-encoding, and the reason generalises.** The owner's capture was a 2,556 KB **PNG carrying a `.webp` extension** at 2880×1800. Browsers sniff content, so it rendered correctly and nothing failed — the file simply lied about its format while shipping 2.5 MB to a featured row on a page whose Performance score was fought from 57 to 80. Its ratio was already exactly 1.6, so a resize to 800×500 needed no crop and lost nothing from frame: **29 KB, a 98.9% saving.** Check format and pixel dimensions on every supplied asset; the extension is not evidence.

### The About column

**It used to render `/profile/avatar.webp` — the exact file the hero's `ProfileCard` renders.** Measured on the built page at the time: two `<img>` elements, one `src`. The owner spotted it, and on 2026-08-22 chose the published paper to replace it.

**It is now a credential rail: the IEEE paper, then the Location/Status list**, with the bio prose beside it. The rail is DOM-second and `md:order-1`, so a phone reads the bio before the citation while a desktop sees the rail on the left. That split is normally a tab-order hazard and is safe here for one specific reason — the rail contains exactly one focusable element, the DOI link.

**"Both columns are 520px" was a false reading, and the mistake is worth keeping.** Grid items stretch to the row height by default, so measuring `getBoundingClientRect().height` on two grid children *always* returns the same number and proves nothing about balance. The owner saw the imbalance on his own screen while that measurement said it was fine. Measuring the content instead — top of the first child to the bottom of the last — showed the prose at 344px against the rail's 520px: **176px of empty column.** To measure whether two grid columns are balanced, measure their contents, never their boxes.

That gap is **40px at 1280** now, closed with a fourth bio paragraph sized from the measurement: 8 lines at 78 characters and a 24px line height. Paragraphs one and two were already at 407 and 419 of 420, so `LIMITS.profile.bio` went 3 → 4 because there was nowhere else to put the words.

**At 768 the sign flips** — prose runs 112px *taller* than the rail, because the narrow column wraps the same prose into far more lines while the rail's height barely moves. That is the better failure mode and it is deliberate: leftover space under a stack of cards reads as padding, leftover space under prose reads as a gap. Perfect balance at every width is not available here.

**The paper's metadata came from Crossref, not from the CV**, and that caught a real error. The CV — and the Education highlight built from it — carried a shortened title, stopping before *"for Explainable Asset Maintenance Recommendations"*. A truncated title in quotation marks, beside a link to the paper it truncates, is exactly the sort of small wrongness a reader who follows the link will notice. The full title plus the rest of that sentence runs past the 160-character highlight limit, so the highlight drops the title rather than clipping it, and About carries it in full. **IEEE Xplore refuses automated fetches; `api.crossref.org/works/<doi>` answers with the registered record** and is the better source anyway.

He is **first author** of four. The card prints the list in publication order because naming only himself would claim more than the paper says, and an invariant asserts he is first rather than merely present.

**That invariant was wrong on its first attempt, in an instructive way.** It asserted `authors[0] === profile.name` and failed: `profile.name` is the short form "Daffa Firasyan" while the paper carries the full legal name. Those differ *on purpose* — `profile.ts` says why — so equality was asserting a sameness the data had deliberately refused. It matches every word of the short name against the first author now.

While looking, a second fault turned up that was nothing to do with redundancy. **The `<img>` declared `width={800} height={800}` for a file that is 320×446** — a square reservation for a 0.717 portrait, so the browser held the wrong box until the image arrived and then reflowed the column. It survived the CLS pass that fixed the desktop 0.245, because nothing about it looks wrong: the numbers simply never matched what `crop-avatar.mjs` writes. Corrected to 320×446. **If a component states an image's dimensions, check them against the file rather than against the spec** — this project's spec says the profile photo is 800×800, and the hero portrait deliberately is not.

The options that were put to the owner, and the reasoning, are worth keeping if this comes back: the strongest candidate is the **IEEE Xplore publication**, because it is the one hard credential on this page that appears nowhere prominent — it is currently a single bullet inside an Education highlight. A skills panel would duplicate Skills, a timeline would duplicate Experience, and a second photo is another asset to source.
...
**Then it did not appear at all, and the cause was neither a crash nor the shader.** Its container in Contact is `hidden lg:block`, because Contact only becomes two columns at `lg` — below that there is no left column for an absolutely positioned globe to occupy. A `display: none` ancestor has **zero area**, an element with zero area **never reports as intersecting**, so the IntersectionObserver that mounts the globe never fires. On a browser window even one pixel under **1024** the globe is simply absent, with nothing logged and nothing broken. Measured: at 1000 the box is `display: none` at 0×0; at 1024 it is `block` at 433×400.

**And it mounts once now, never unmounting, which is a leak fix.** `useOnScreen` toggles, so scrolling past Contact and back unmounted and remounted the globe — and React hands a *new* `<canvas>` to each mount. **cobe never releases its WebGL context:** `destroy()` stops the render loop, and `loseContext` appears nowhere in the package, verified rather than assumed. Every remount therefore stranded a live context on a discarded canvas until the browser began evicting the oldest, which is the `Too many active WebGL contexts` warning. `ready` latches true and the observer disconnects on first hit — the shape `SplineRobot` reached for the same reason.

Releasing the context in cleanup was the other candidate repair and is the one this project has already been burnt by: `SplashCursor` called `loseContext()` on a canvas React reuses across StrictMode's mount/cleanup/mount, and the second mount got a dead context and drew nothing for the life of the page.

**Installing a dependency while the dev server runs breaks that server, not the code.** After `cobe` was installed and then downgraded under a live `npm run dev`, Vite served `504 (Outdated Optimize Dep)` for it and the dynamic import of `Globe.tsx` failed — on the dev server only. The production build was fine throughout, which is why nothing here caught it: this project's browser checks run against `npm run preview` on 4173. **The fix is to delete `node_modules/.vite` and restart the dev server**, and it is a rebuildable cache so deleting it costs nothing.

**That failure is also the first proof `DecorationBoundary` works.** The console shows exactly the intended sequence: the import fails, React reports it caught the error, and `[decoration: globe] disabled after an error` is logged — while the page stays up. Before the boundary existed, a failed dynamic import in a decoration would have unmounted the whole tree.

**`cobe` is pinned exactly**, not with a caret. `^0.6.5` is safe on paper — a caret on a `0.x` version cannot reach `2.x` — but the failure mode if it ever did is a globe that draws one frame and freezes with nothing thrown, and that is not worth leaving to semver rules.

**It could not be dragged, and `pointer-events-none` was why.** The slot's container in Contact carried it, correctly, for the two things that held the slot before: the Spline robot and the orb both read the **window** and normalised against their own rect, so neither needed the box to be hittable. The globe does not work that way — it listens on its own canvas for `mousedown`/`mousemove`. With pointer events disabled on an ancestor nothing ever reached it, and the canvas was sitting there carrying `cursor: grab` the whole time.

Removing it was measured first, because an absolutely positioned box across a column is exactly the shape that swallows clicks silently: it begins **15px below** the social links so it covers none of them, and the only thing it overlaps is **40px of footer**, which has no links, no buttons and nothing focusable — just the credit marquee. Verified after: a hit test at the canvas centre resolves to the canvas, a real `mousedown` reaches its listener, and the cursor flips to `grabbing`, which is the component's own drag handler engaging.

**A third wrong reading of mine, in the same check.** Probing whether the globe swallowed the wheel returned `defaultPrevented: true` and looked like a bug. It is not: **Lenis preventDefaults wheel events page-wide** — over the body and over ordinary paragraphs just the same — because that is how it drives smooth scrolling. `enableZoom={false}` was working correctly. A `defaultPrevented` reading means nothing on this page unless it is compared against an ordinary element.

**Two of my own readings were wrong in this episode and both are worth keeping.** I rejected the WebGL context-exhaustion theory after a probe granted 24 contexts in a burst — the probe was fresh canvases created and released in one synchronous loop, which is not the same thing as contexts stranded on discarded elements, and the browser then printed the exhaustion warning outright. And I read `getContext('webgl2') === null` on the starfield as a dead context, when asking for webgl2 on a webgl1 canvas returns null regardless. **In this pane the console also keeps its history across navigations**, so a warning list is not evidence about the load you just did.
