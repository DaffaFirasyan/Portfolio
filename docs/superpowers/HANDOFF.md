...
| **`village-portal.webp` is the wrong screenshot** | `public/projects/village-portal.webp` | **Still outstanding.** Byte-identical to `simpel-ibs.webp`, and it shows the SimpelBS homepage. A featured row is displaying another project's site. Needs a real `banjarsarigarut.id` capture at 800×500. |
| **`Neo4j` and `FastAPI` are on a featured card but not in Skills** | [`src/data/skills.ts`](../../src/data/skills.ts) | AssetMind's chip row names both; the Skills section names neither, so a reader filtering by skill cannot find them. Adding them needs a `level`, which is the owner's to judge. |
