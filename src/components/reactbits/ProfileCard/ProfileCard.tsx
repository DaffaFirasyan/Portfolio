...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const tiltEngine = useMemo<TiltEngine | null>(() => {
    ...
    const step = (ts: number): void => {
      ...
      // `|| document.hasFocus()` removed. Upstream keeps re-scheduling for as
      // long as the tab is focused, which is the normal case — so the else
      // branch below looks like an idle path and almost never runs. That is a
      // permanent 60fps loop in the hero, the first thing rendered and the
      // element LCP is measured against. Settling is the only reason to keep
      // animating.
      if (stillFar) {
      ...
      <div ref={shellRef} className="relative z-[1] group">
        ...
        >
          ...
          >
            ...
            >
              {/* Gated on showUserInfo, which upstream does not do — the flag
                  closes before this block, so turning it off still rendered the
                  name and the title. In this project that would print the name
                  a second time beside the page's h1 and drop an h3 into the
                  hero with no h2 between them. */}
              {showUserInfo && (
